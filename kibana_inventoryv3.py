...
Usage:
    # Using deployment name and object ID:
    python kibana_inventory.py --deployment prod --object_id your_object_id
    
    # Using deployment name for full inventory:
    python kibana_inventory.py --deployment qa --output_format json
...
deployment_config = {
    "prod": {
        "kibana_url": "https://prod_kibana_url_endpoint",
        "api_key": "prod_api_key"
    },
    "qa": {
        "kibana_url": "https://qa_kibana_url_endpoint",
        "api_key": "qa_api_key"
    },
    "dev": {
        "kibana_url": "https://dev_kibana_url_endpoint",
        "api_key": "dev_api_key"
    },
    "mon": {
        "kibana_url": "https://mon_kibana_url_endpoint",
        "api_key": "mon_api_key"
    },
    "infosec": {
        "kibana_url": "https://infosec_kibana_url_endpoint",
        "api_key": "infosec_api_key"
    },
    "ccs": {
        "kibana_url": "https://ccs_kibana_url_endpoint",
        "api_key": "ccs_api_key"
    }
}
...
# Setup Log file
def setup_log_file(timestamp, deployment=None):
    # Create the log file name with the EST timestamp and deployment
    if deployment:
        log_file_name = f"kibana_inventory_{deployment}_log_{timestamp}.log"
    else:
...
def get_deployment_config(deployment_name):
    """
    Get the Kibana URL and API key for a specific deployment.
    
    Args:
        deployment_name (str): Name of the deployment (prod, qa, dev, etc.)
        
    Returns:
        tuple: (kibana_url, api_key) or (None, None) if deployment not found
    """
    if deployment_name not in deployment_config:
        available_deployments = list(deployment_config.keys())
        logging.error(f"Unknown deployment: {deployment_name}")
        logging.error(f"Available deployments: {', '.join(available_deployments)}")
        return None, None
    
    config = deployment_config[deployment_name]
    return config["kibana_url"], config["api_key"]


def debug_object_structure(obj, obj_type, space_id):
    """
    Debug function to print the actual structure of Kibana objects.
    This helps identify the correct path to title and description fields.
    
    Args:
        obj (dict): The Kibana object
        obj_type (str): Type of the object
        space_id (str): Space ID where object was found
    """
    logging.info(f"=== DEBUG: Object Structure for {obj_type} in space {space_id} ===")
    logging.info(f"Object ID: {obj.get('id', 'Unknown')}")
    logging.info(f"Object Type: {obj.get('type', 'Unknown')}")
    logging.info(f"Top-level keys: {sorted(list(obj.keys()))}")
    
    # Check attributes structure (this is where Kibana stores most metadata)
    attributes = obj.get("attributes", {})
    if attributes:
        logging.info(f"Attributes keys: {sorted(list(attributes.keys()))}")
        
        # Show actual title and description values
        title = attributes.get("title")
        description = attributes.get("description")
        
        logging.info(f"attributes.title: '{title}'" + (" (FOUND)" if title else " (NOT FOUND)"))
        logging.info(f"attributes.description: '{description}'" + (" (FOUND)" if description else " (EMPTY/NOT FOUND)"))
        
        # Check for other potential title fields
        other_fields = ['name', 'displayName', 'label']
        for field in other_fields:
            value = attributes.get(field)
            if value:
                logging.info(f"attributes.{field}: '{value}' (POTENTIAL TITLE)")
        ...
        logging.info("❌ No 'attributes' key found - this is unusual for Kibana objects")
    
    # Final extraction test
    extracted_title = extract_object_title(obj)
    extracted_desc = extract_object_description(obj)
    logging.info(f"✅ EXTRACTED TITLE: '{extracted_title}'")
    logging.info(f"✅ EXTRACTED DESCRIPTION: '{extracted_desc}'")
    logging.info("=== END DEBUG ===\n")


# Enhanced function to get object title with multiple fallbacks
def extract_object_title(obj):
    """
    Extract title from Kibana object using multiple strategies.
    Based on actual Kibana object structure analysis.
    
    Args:
        obj (dict): Kibana object
        
    Returns:
        str: Object title or fallback value
    """
    # Primary strategy: attributes.title (this is where Kibana stores titles)
    attributes = obj.get("attributes", {})
    if attributes.get("title"):
        return str(attributes["title"]).strip()
    
    # Secondary strategies for edge cases
    title_fields = ['name', 'displayName', 'label']
    for field in title_fields:
        if attributes.get(field):
            return str(attributes[field]).strip()
    
    # Top-level fallbacks (rare but possible)
    if obj.get('title'):
    ...
    type_name = type_names.get(obj_type, obj_type.title())
    return f"{type_name}-{obj_id}"


# Enhanced function to get object description
def extract_object_description(obj):
    """
    Extract description from Kibana object using multiple strategies.
    Based on actual Kibana object structure analysis.
    ...
    Returns:
        str: Object description or empty string
    """
    # Primary strategy: attributes.description
    attributes = obj.get("attributes", {})
    if attributes.get("description"):
        desc = str(attributes["description"]).strip()
        return desc if desc else ""
    
    # Secondary strategies
    desc_fields = ['desc', 'summary', 'note']
    for field in desc_fields:
        if attributes.get(field):
            desc = str(attributes[field]).strip()
            return desc if desc else ""
...
def get_kibana_objects_by_type(headers, kibana_url, space_id, object_types):
    ...
    for obj_type in object_types:
        params = {
            'type': obj_type,
            'per_page': 10000,
            # Remove fields parameter to get all attributes
        ...
        try:
            ...
            for obj in objects:
                ...
                title = extract_object_title(obj)
                description = extract_object_description(obj)
                
                # Optional: Enable debug mode for troubleshooting
                if hasattr(main, '_debug_mode') and main._debug_mode and len(all_objects) < 5:
                    debug_object_structure(obj, obj_type, space_id)
            ...
            try:
                ...
                for obj in objects:
                    if obj["id"] == target_object_id:
                        ...
                        # Enable debug for found objects
                        debug_object_structure(obj, obj_type, space_id)
...
def display_search_results(matching_objects, target_object_id, deployment_name=None):
    """
    ...
    Args:
        ...
        deployment_name (str): Name of the deployment being searched
    """
    deployment_info = f" in {deployment_name.upper()} deployment" if deployment_name else ""
    
    print("\n" + "="*80)
    print(f"SEARCH RESULTS FOR OBJECT ID: {target_object_id}{deployment_info}")
    print("="*80)
    
    if not matching_objects:
        print(f"❌ No objects found with ID: {target_object_id}{deployment_info}")
...
def validate_arguments(args):
    """
    Validate command line arguments to ensure proper combinations are used.
    ...
    """
    # Check if deployment method is being used
    using_deployment = args.deployment is not None
    
    # Check if legacy method is being used
    using_legacy = args.kibana_url is not None and args.api_key is not None
    
    if using_deployment and using_legacy:
        logging.error("Cannot use both --deployment and --kibana_url/--api_key at the same time.")
        logging.error("Use either deployment-based approach OR legacy approach, not both.")
        return False
    
    if not using_deployment and not using_legacy:
        logging.error("Must provide either --deployment OR both --kibana_url and --api_key")
        return False
    
    if using_legacy and (args.kibana_url is None or args.api_key is None):
        logging.error("When using legacy approach, both --kibana_url and --api_key are required")
        return False
...
def main():
    ...
    # Deployment-based arguments (new approach)
    parser.add_argument('--deployment', 
                       choices=list(deployment_config.keys()), 
                       help=f'Deployment name. Available: {", ".join(deployment_config.keys())}')
    
    # Legacy arguments (maintained for backward compatibility)
    parser.add_argument('--kibana_url', help='Kibana URL (e.g., https://your-kibana-url)')
    parser.add_argument('--api_key', help='Kibana API key for authentication')
    ...
    parser.add_argument('--debug', action='store_true', 
                       help='Enable debug mode to show object structure details')
    
    args = parser.parse_args()
    
    # Set debug mode as module attribute for access in other functions
    main._debug_mode = args.debug
    ...
    log_file_name = setup_log_file(timestamp, args.deployment)
    setup_logging(log_file_name)
    
    # Determine Kibana URL and API key
    if args.deployment:
        # Use deployment configuration
        kibana_url, api_key = get_deployment_config(args.deployment)
        if not kibana_url or not api_key:
            return 1
        logging.info(f"Using deployment configuration for: {args.deployment}")
    else:
        # Use legacy approach
        kibana_url = args.kibana_url
        api_key = args.api_key
        logging.info("Using legacy configuration (direct URL and API key)")
    ...
    if args.object_id:
        deployment_info = f" in {args.deployment} deployment" if args.deployment else ""
        logging.info(f"Starting search for object ID: {args.object_id}{deployment_info}")
        matching_objects = search_object_by_id(headers, kibana_url, args.object_id)
        display_search_results(matching_objects, args.object_id, args.deployment)
        ...
        if matching_objects and args.output_file:
            ...
            deployment_suffix = f"_{args.deployment}" if args.deployment else ""
            output_filename = f"{args.output_file}{deployment_suffix}_search_results.json"
            try:
    ...
    deployment_info = f" for {args.deployment} deployment" if args.deployment else ""
    logging.info(f"Starting Kibana objects inventory{deployment_info}...")
    inventory = generate_kibana_inventory(headers, kibana_url)
    ...
    if args.output_file:
        ...
        deployment_suffix = f"_{args.deployment}" if args.deployment else ""
        output_base = f"kibana_inventory{deployment_suffix}_{timestamp}"
